...
// ════════════════════PLUGGINS SUCCESS◽◽◽◽
        console.log(
            chalk.green.bold('👸Stefanie👸 working!')
       );
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            if (conn.user.jid.startsWith('90')) { 
                ...
                const biography = '📅 ' + utch + '\n⌚ ' + ov_time + '\n\n⏱ Auto Bio By... 🚀powered By Stefanie'
                ...
                const biography = '📅 ' + utch + '\n⌚ ' + ov_time +'\n\n⏱ Auto Bio B... 🚀powered By Stefanie'
                await conn.setStatus(biography)
            }
...
// ════════════════════AUTO BIO◽◽◽◽◽    
    setInterval(async () => { 
        ...
        while (getGMTh == 19 && getGMTm == 1) {
            var announce = ''
            if (config.LANG == 'EN') announce = '📢👸stefanie👸\n📢Announcement SYSTEM 🔘'
            if (config.LANG == 'SI') announce = '📢👸Stefanie👸\n📢නිවේදන පද්ධතිය 🔘'
            if (config.LANG == 'ID') announce = '📢👸Stefanie👸\n📢Announcement System 🔘'
            
            let video = 'https://imgur.com/u9LLLGV.mp4'
            let image = 'https://telegra.ph/file/7a8c47c246bd16d90623d.jpg'
...
*/
                        try {
                            ...
                            if (config.LANG == 'EN') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/Stefanie.png"), MessageType.image, { caption: '*🚀Stefanie*  WORKING PERFECTLY !!\n\n▷ _This is your LOG number Dont Try Command here_\n▷Also You Can join Our Support group More Help.\n_👾Support 01▷https://chat.whatsapp.com/KlpSnFrspoaEu2kRrjx8v4\n\n*Error:* ```' + error + '```\n\n' });
                                
                            } else if (config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, fs.readFileSync("./src/image/Stefanie.png"), MessageType.image, { caption: '*🚀Stefanie*  නිසි ලෙස ක්‍රියා කරයි!!\n\n▷ _මෙය ඔබගේ LOG අංකයයි මෙහි විධන භාවිතයෙන් වළකින්න_\n▷ඔබට යම් ගැටලුවක් ඇත්නම් අපගේ සහය සමූහට ලිවිය හැක.\n_👾Support 01▷https://chat.whatsapp.com/KlpSnFrspoaEu2kRrjx8v4\n\n*දෝෂය:* ```' + error + '```\n\n' });
